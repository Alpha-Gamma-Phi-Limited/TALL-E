...
class RetailerProduct(Base):
    ...
    image_url: Mapped[str | None] = mapped_column(Text)
    raw_attributes: Mapped[dict[str, object]] = mapped_column(JSON, default=dict)
...
class ProductOverride(Base):
    ...
    id: Mapped[str] = mapped_column(String(36), primary_key=True, default=new_id)
    retailer_product_id: Mapped[str] = mapped_column(ForeignKey("retailer_products.id"), unique=True)
